...
export default function DoctorsPageClient({
  ...
  // Filter doctors by name or specialisation
  ...
  function handleSearch(e: React.FormEvent) {
    ...
    setQuery(trimmed)
    if (trimmed) {
      router.replace(`/doctors?q=${encodeURIComponent(trimmed)}`)
    } else {
  ...
  return (
    <div className="min-h-screen bg-[#EFECE3]">
      {/* ── Header ─────────────────────────────────────────────── */}
      <div className="border-b-[3px] border-[#203C67] bg-white px-6 py-8">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-[#203C67] font-bold text-3xl tracking-tight">
            Find a Doctor
          </h1>
          ...
          <form onSubmit={handleSearch} className="mt-5 flex gap-2 max-w-xl">
            <div className="flex-1 flex items-center gap-2 bg-[#F3F6FB] border-2 border-gray-200 rounded-xl px-3 h-11 focus-within:border-[#203C67] transition-colors">
              ...
              />
              {inputVal && (
                <button type="button" onClick={handleClear} className="text-gray-400 hover:text-gray-600 cursor-pointer">
            ...
            <button
              type="submit"
              className="px-5 h-11 bg-[#203C67] text-white text-sm font-semibold rounded-xl hover:bg-[#162d50] transition-colors cursor-pointer"
            >
              Search
            </button>
          </form>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <button
              onClick={handleClear}
              className="mt-2 text-sm text-[#203C67] underline underline-offset-2 font-semibold cursor-pointer"
            >
              Clear search
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((doctor) => (
              <DoctorCard key={doctor.$id} doctor={doctor} userId={authUser?.$id ?? ''} patientId={fullUser?.$id ?? ''} authUser={authUser} fullUser={fullUser} />
            ))}
          </div>
        )}
      </div>
