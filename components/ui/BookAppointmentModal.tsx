...
const BookAppointmentModal = ({
  ...
  variant,
  DateToday,
  doctor: doctorProp,       // now optional
  userId,
  patientId,
  authUser,
  fullUser,
  ...
  // ── Active doctor = prop (if given) else picker selection ─────────────────
  const doctor = selectedDoctor

  // ── Fetch all doctors when picker is needed ────────────────────────────────
  useEffect(() => {
  ...
  // ── Fetch slots when doctor is known ──────────────────────────────────────
   useEffect(() => {
    if (!doctor) return
  ...
  // ── Replace handleConfirm in BookAppointmentModal with this ─────────────────
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      // Always update local bookedSlots if the server returned them,
      // so the UI stays consistent regardless of outcome.
      ...
      if (!result.success) {
        // UNKNOWN or any other server error
  ...
  // Steps shown in indicator: if doctor was pre-selected, show 1-2; else show D-1-2
  ...
  return (
    <>
      ...
      {variant === 'ghost' ? (
        ...
        // Full-width dark button for use inside the sidebar
        <button
          onClick={handleBookingButton}
        ...
        >
          <div className="min-w-0">
            <p className="text-[12px] font-bold text-white leading-tight text-left">
              {text || 'Book Appointment'}
            </p>
            <p className="text-[10px] text-blue-200/60 mt-0.5 text-left">Find &amp; schedule a visit</p>
          </div>
          <div className="shrink-0 w-7 h-7 rounded-lg bg-white/10 group-hover:bg-white/20 flex items-center justify-center transition-colors">
            ...
            {doctor ? (
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[#8FABD4] text-[11px] sm:text-[12px] mt-1 truncate">{doctor.specialization?.length ? doctor.specialization[0] : "General Physician"}</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="overflow-y-auto max-h-[calc(100dvh-88px)] sm:max-h-[calc(95vh-80px)]">
            ...
            {step === 0 && (
              <div className="p-4 sm:p-6 flex flex-col gap-4">
                <p className="text-[11px] font-semibold text-[#343641]/60 uppercase tracking-wider">
                  Choose a Doctor
                </p>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-[#c8d4de] bg-white text-[13px] text-[#343641] placeholder:text-[#bbc3cf] focus:outline-none focus:border-[#4A70A9] transition-colors"
                  />
                </div>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isSelected
                              ? "bg-[#dde8f5] border-[#4A70A9] shadow-sm"
                              : "bg-white border-[#e2ddd4] hover:border-[#4A70A9] hover:bg-[#f5f8fd]"
                          )}
                        >
                          {/* Avatar */}
                          <div className="shrink-0">
                            {doc.profilePic ? (
                              <Image
                                src={doc.profilePic} alt={doc.name}
                                height={100} width={100}
                                className="w-11 h-11 rounded-xl object-cover"
                              />
                            ) : (
                              <div className="w-11 h-11 rounded-xl bg-[#dde8f5] flex items-center justify-center text-[13px] font-bold text-[#203C67]">
                                {initials}
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[13px] font-semibold text-[#1a2535] truncate">
                              Dr. {doc.name}
                            </p>
              ...
              <div className="p-4 sm:p-6 flex flex-col gap-5 sm:gap-6">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-[340px_1fr] gap-5">
                  ...
                  <div className="bg-white/70 backdrop-blur-md border border-[#d8e1ea] rounded-3xl p-3 sm:p-4 shadow-sm flex flex-col lg:flex-row gap-4 lg:gap-5">
                    ...
                    <div className="flex-col flex w-full lg:w-auto">
                      <p className="text-[11px] font-semibold text-[#7c8da3] uppercase tracking-[0.18em] mb-4">
                        Select Date
                      </p>
                      ...
                      <div className="flex gap-1 overflow-x-auto pb-1 mb-4 no-scrollbar">
                        {QUICK_DATES.map((qd) => (
                          <button
                            ...
                            className="shrink-0 px-2 py-1 rounded-full text-[11px] font-medium border border-[#d8e1ea] bg-white hover:border-[#4A70A9] hover:text-[#4A70A9] transition-all"
                          >
                            {qd.label}
                          </button>
                        ))}
                        ...
                        <CardContent className="p-2 sm:p-3 overflow-x-auto">
                          <Calendar
                            ...
                            classNames={{
                              ...
                              day:         "h-7 w-12 sm:h-8 sm:w-8 rounded-xl text-[9px] sm:text-[10px] font-medium transition-all flex items-center justify-center bg-[#f7f9fc] hover:bg-[#eaf2fc] hover:text-[#4A70A9]",
                              day_selected:"bg-[#4A70A9] text-white hover:bg-[#4A70A9] hover:text-white",
                              day_today:   "border border-[#4A70A9] text-[#4A70A9] bg-white",
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-between sm:items-center">
                  {/* Back to doctor picker if no pre-selected doctor */}
                  {!doctorProp ? (
                    <button
                      onClick={() => setStep(0)}
                      className="text-[12px] text-[#4A70A9] font-medium hover:underline flex items-center gap-1"
                    >
                      ← Change doctor
                    </button>
                  ) : (
                ...
                <div className="bg-white rounded-2xl border border-[#c8d4de] overflow-hidden">
                  <div className="flex items-center gap-4 px-4 sm:px-6 py-5 border-b border-[#e8ecef]">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[#8FABD4] text-[11px] sm:text-[12px] mt-0.5 truncate">{doctor?.specialization?.length ? doctor.specialization[0] : "General Physician"}</p>
                    </div>
                  </div>
            ...
            {step === 3 && (
              <div className="flex flex-col items-center justify-center p-6 sm:p-8 text-center">
                ...
                <Image
                  src="/assets/gifs/success.gif" alt="success"
